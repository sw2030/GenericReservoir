...
function Solve_SPE10_CPR(m::Reservoir_Model{T}, t_init, Δt, g_guess, n_steps; tol_relnorm=1e-3, tol_gmres=1e-2, n_restart=20, n_iter=50, n_prec=7, step_init=0, iftol2=false, arg2=(1e-1, 100, 30, 31), CPR_tol=(1e-2, 1e-2), CPR_iter=(50, 50), CPR_prec=(15, 7), CPR_restart=(20, 20)) where {T}
    ...
    for steps in 1:n_steps
	    ...
	    print("LinSolve start...")
	    gmresresult, inner_iter = CPR_LSPS(JAC, precP, precE, RES, CPR_tol, CPR_iter, CPR_prec, CPR_restart)
	    gmresresult2 = gmres(JAC, RES, n_restart;maxiter=n_iter, M=(t->GenericReservoir.lsps_prec(precP, precE, n_prec, t)), tol=tol_gmres)[3]
	    gmresitercount += gmresresult[3]
            gmresnumcount  += 1
            gmreserr = gmresresult[4]
	    println("...LinSolve done  ||  Iter : ",(inner_iter, gmresresult[3], gmresresult2), " || rel_err : ",gmreserr)
	    if (iftol2 && gmreserr>arg2[1]) ## Option for having expensive solver
                print("(opt2) start...")
                gmresresult = gmres(JAC, RES, arg2[2];tol=tol_gmres, maxiter=arg2[3], M=(t->lsps_prec(precP, precE, arg2[4], t)), ifprint=false)
                gmresitercount += gmresresult[3]
